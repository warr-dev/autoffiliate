<script lang="ts">
    ...
    let filter = $state<'all' | 'draft' | 'approved' | 'published' | 'failed'>('all');
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-gray-400 text-xs mt-1">{posts.length} total recorded posts</p>
            </div>

            <div class="flex flex-wrap gap-1 bg-gray-900/70 p-1.5 rounded-xl border border-gray-800/80 text-xs shadow-md">
                {#each (['all', 'draft', 'approved', 'published', 'failed'] as const) as f}
                    <button
                        ...
                        class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 capitalize cursor-pointer
                            {filter === f ? 'bg-gray-800 text-gray-100 shadow-sm border border-gray-700/60' : 'text-gray-400 hover:text-gray-200'}"
                    >
                        {f === 'all' ? 'All' :
                         f === 'draft' ? '✎ Drafts' :
                         f === 'approved' ? '⌛ Approved' :
        ...
        {#if filtered.length === 0}
            <div class="p-16 text-center bg-gray-900/60 border border-gray-800/80 rounded-2xl shadow-xl">
                <div class="text-5xl mb-4 opacity-30">📭</div>
                <p class="text-gray-400 font-medium">No posts found for "{filter}" filter</p>
            </div>
        {:else}
            <div class="space-y-2">
                {#each filtered as post (post.id)}
                    <div class="bg-gray-900/70 backdrop-blur-xl border border-gray-800/80 hover:border-indigo-500/40 p-4 rounded-2xl flex items-center justify-between transition-all shadow-md">
                        <div class="flex items-center gap-4 min-w-0">
                            ...
                            <div class="min-w-0">
                                <p class="font-bold text-sm text-gray-200 truncate">
                                    {post.product_title || 'Shopee Product Deal'}
                                </p>
                                <p class="text-xs text-indigo-400 font-mono truncate mt-0.5">
                                    {post.affiliate_url}
                                </p>
                        ...
                        <div class="flex items-center gap-3 flex-shrink-0 ml-3">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide uppercase
                                {post.status === 'published' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                                 post.status === 'approved' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' :
                                ...
                                <span class="w-1.5 h-1.5 rounded-full
                                    {post.status === 'published' ? 'bg-emerald-400' :
                                     post.status === 'approved' ? 'bg-indigo-400 animate-ping' :
